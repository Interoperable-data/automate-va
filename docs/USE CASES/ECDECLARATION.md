...
Example: [Schunk Carbon's DOC for Pantograph contact strips](https://eradis.era.europa.eu/interop_docs/ecDecl/view.aspx?id=8917&DocumentType=ECDeclCnf). This EC DOC contains many 2CH1 design examination certificates. Internally (see /OSSDOCS#doc-ecDecl-8917>), the property to link them is `dcterms:requires`, while the proposed ontology prescribes the use of `era:CABEvidence`, a class of which all instances are linked to by the `rdf:member` property. This EC Declaration has been elaborated [here](../../TTL/examples/ecDecl-8917.ttl).
