...
export async function getPropertiesFromTypeRegistration(
  ...
  try {
    ...
    const dataset = await getSolidDataset(registration.inContainer, {
      fetch: fetch,
    });
    const things = getThingAll(dataset);
...
export async function addProcessSubContainers(
  ...
  try {
    ...
    const dataset = await getSolidDataset(instanceContainer, {
      fetch: fetch,
    });
...
export async function addTaskResources(processContainer: URL): Promise<void> {
  try {
    ...
    const dataset = await getSolidDataset(processContainer.href, {
      fetch: fetch,
    });
    const resourceUrls = getContainedResourceUrlAll(dataset);
    ...
    for (const resourceUrl of resourceUrls) {
      if (!isContainer(resourceUrl)) {
        ...
        const taskDataset = await getSolidDataset(resourceUrl, {
          fetch: fetch,
        });
...
export async function getTypeRegistrationsFromContainers(
  ...
  for (const container of containers) {
    ...
    try {
      ...
      const dataset = await getSolidDataset(container.href, {
        fetch: fetch,
      });
      const things = getThingAll(dataset);
